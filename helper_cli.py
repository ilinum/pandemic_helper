...
def mainloop():
    ...
    while user_option != "q":
        ...
        if user_option == "quit" or user_option == "q":
            save_state((infection_deck, discard_pile), "auto_save.pkl")
            break
        elif user_option == "save" or user_option == "s":
            save_state((infection_deck, discard_pile), "manual_save.pkl")
        elif user_option.startswith("load") or user_option.startswith("l "):
            l = user_option.split(" ")
            if len(l) == 1:
                infection_deck, discard_pile = load_state("manual_save.pkl")
                print_decks()
            elif len(l) == 2:
                if l[1] == "manual":
                    infection_deck, discard_pile = load_state("manual_save.pkl")
                    ...
                    infection_deck, discard_pile = load_state("auto_save.pkl")
                    print_decks()
                else:
